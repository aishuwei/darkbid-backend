...
import os, logging
import requests
from backend.nosql_client import TokenManager
...
class StorageClient:
    ...
    def upload(self, local_path: str, cloud_path: str) -> bool:
        ...
        resp = requests.post(f"{self._base_url}/v1/storages/get-objects-upload-info",
                             headers=self._headers(), json=[{"objectId": cloud_path}], timeout=15)
        data = resp.json()
        ...
        with open(local_path, "rb") as f:
            file_data = f.read()
        upload_resp = requests.put(item["uploadUrl"], data=file_data, headers={
    ...
    def get_download_url(self, cloud_path: str, expires=3600) -> str | None:
        resp = requests.post(f"{self._base_url}/v1/storages/get-objects-download-info",
                             headers=self._headers(), json=[{"cloudObjectId": cloud_path}], timeout=15)
        data = resp.json()
        if not isinstance(data, list) or not data:
            return None
        item = data[0]
    ...
    def delete(self, cloud_path: str) -> bool:
        resp = requests.post(f"{self._base_url}/v1/storages/delete-objects",
                             headers=self._headers(), json=[{"cloudObjectId": cloud_path}], timeout=15)
        data = resp.json()
